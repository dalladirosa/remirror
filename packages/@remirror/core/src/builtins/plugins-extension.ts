...
export class PluginsExtension extends PlainExtension {
   ...
   */
  readonly #applyStateHandlers: Array<(parameter: ApplyStateLifecycleParameter) => void> = [];

  /**
   * Called when the state is first initialized.
   */
  readonly #initStateHandlers: Array<(state: EditorState) => void> = [];

  /**
   * Handlers for the `onAppendTransaction` lifecycle method.
   */
  readonly #appendTransactionHandlers: Array<(parameter: AppendLifecycleParameter) => void> = [];
  ...
  onCreate(): void {
    ...
    for (const extension of this.store.extensions) {
      if (extension.onApplyState) {
        this.#applyStateHandlers.push(extension.onApplyState);
      }
