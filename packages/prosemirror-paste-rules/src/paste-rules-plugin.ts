  ...
  return new Plugin({
    ...
    props: {
      ...
      handlePaste: (view, event) => {
        ...
        const files: File[] = [...clipboardData.items]
        ...
        for (const { fileHandler } of filePasteRules) {
          if (fileHandler({ event, files, selection, view })) {
            return true;
          }
        }
...
export interface FileHandlerParameter {
